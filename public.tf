resource "aws_subnet" "eks_subenet_public_1a" {
...
resource "aws_subnet" "eks_subenet_public_1b" {
  vpc_id                  = aws_vpc.eks_vpc.id
...
resource "aws_route_table_association" "eks_rtb_assoc_1a" {
  subnet_id      = aws_subnet.eks_subenet_public_1a.id
  route_table_id = aws_route_table.eks_public_route_table.id
}

resource "aws_route_table_association" "eks_rtb_assoc_1b" {
  subnet_id      = aws_subnet.eks_subenet_public_1b.id
  route_table_id = aws_route_table.eks_public_route_table.id
}
